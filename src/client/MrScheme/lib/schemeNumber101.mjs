...
import { BigInteger } from "./biginteger101.mjs";
/*
...
*/
export const SchemeNumber = (function() {
...
function implementCoreLibrary(plugins) {
    ...
    function onPluginsChanged(plugins, changed) {
        ...
        function getComplexConstant(x, y) {
            try {
              if (typeof exactRectangular != "undefined") {
...
function implementRnrsBase(plugins) {
    ...
    var fn = {
        ...
        div             : makeDivMod(false, 0),  // QNC useless now
        mod             : makeDivMod(false, 1),  // QNC useless now
        remainder       : makeDivMod(false, 3),  // QNC 
        modulo          : makeDivMod(false, 4),  // QNC
    ...
    function makeDivMod(is0, which) {
        function divMod(x, y) {
            ...
            if (!is0) {
                var themod = mod(x, y);
                var oppmod = subtract(abs(y), themod);
                switch (which) {
                case 0: return div(x, y);
                case 1: return themod;                    // QNC no longer used
                case 2: default: return divAndMod(x, y);
                case 3: return ((isPositive(x)) ? themod :
                                (subtract(ZERO, oppmod))); // QNC remainder
                case 4: return ((isPositive(y)) ? themod :
                                (subtract(ZERO, oppmod))); // QNC modulo
                }
            }
